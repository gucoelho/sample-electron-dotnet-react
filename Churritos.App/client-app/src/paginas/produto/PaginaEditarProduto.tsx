...
const PaginaEditarProduto = ({ match: { params }, history }: any) => {
    ...
    const [categoriaSelecionada, setCategoriaSelecionada] = React.useState<number>()
    ...
    const editarProduto = async () => {
        const rawResponse = await fetch(`/api/produto/${params.id}`, {
            method: 'PUT',
            headers: {
        ...
        {!loading &&
            <Fade in>
                <form noValidate autoComplete="off">
                    <ContainerEdicao>
                        <Button variant="contained" color="primary" onClick={() => editarProduto()}>Salvar</Button>
                    </ContainerEdicao>
                    <ContainerDados>
                        <Typography color="secondary" variant="h6" gutterBottom>Dados do produto: </Typography>
                        <SecaoEdicao>                            <Grid container spacing={3}>
                            <Grid item xs={2}>
                                <TextField
                                    select
                                    label="Categoria"
